{ config, lib, pkgs, system, userName, hostName, pkgs-unstable, pkgs-otbr ? null, inputs ? {}, ... }:
let
  hasNur = inputs ? nur;
  hasYazi = inputs ? yazi;
  hasWezterm = inputs ? wezterm;
  hasHyprland = inputs ? hyprland;
  hasCatppuccinThemes = inputs ? catppuccin-themes;
  hasPkgsOtbr = pkgs-otbr != null;
in {
  nix = lib.mkIf config.nix.enable {
    optimise.automatic = true;
    gc = {
      automatic = true;
      options = "--delete-older-than 30d";
    };
    settings = {
      builders-use-substitutes = true;
      extra-experimental-features = [ "flakes" "nix-command" ];
      substituters = [
        "https://cache.nixos.org"
        "https://nix-community.cachix.org"
        "https://nixos-raspberrypi.cachix.org"
      ] ++ lib.optionals hasHyprland [
        "https://hyprland.cachix.org"
      ] ++ lib.optionals hasWezterm [
        "https://wezterm.cachix.org"
      ] ++ lib.optionals hasYazi [
        "https://yazi.cachix.org"
      ];
      trusted-public-keys = [
        "cache.nixos.org-1:6NCHdD59X431o0gWypbMrAURkbJ16ZPMQFGspcDShjY="
        "nix-community.cachix.org-1:mB9FSh9qf2dCimDSUo8Zy7bkq5CX+/rkCWyvRCYg3Fs="
        "nixos-raspberrypi.cachix.org-1:4iMO9LXa8BqhU+Rpg6LQKiGa2lsNh/j2oiYLNOQ5sPI="
      ] ++ lib.optionals hasHyprland [
        "hyprland.cachix.org-1:a7pgxzMz7+chwVL3/pzj6jIBMioiJM7ypFP8PwtkuGc="
      ] ++ lib.optionals hasWezterm [
        "wezterm.cachix.org-1:kAbhjYUC9qvblTE+s7S+kl5XM1zVa4skO+E/1IDWdH0="
      ] ++ lib.optionals hasYazi [
        "yazi.cachix.org-1:Dcdz63NZKfvUCbDGngQDAZq6kOroIrFoyO064uvLh8k="
      ];
    };
  };
  nixpkgs = {
    hostPlatform = system;
    config = {
      allowUnfree = true;
      allowBroken = true;
    };
    overlays =
      lib.optionals hasNur [
        inputs.nur.overlays.default
      ] ++ lib.optionals hasYazi [
        inputs.yazi.overlays.default
      ] ++ lib.optionals hasCatppuccinThemes [
        inputs.catppuccin-themes.overlays.default
      ] ++ [
        (final: prev: { boehm-gc = prev.boehm-gc.overrideAttrs (_: { doCheck = false; }); })
      ] ++ lib.optionals hasHyprland [
        (final: prev: {
          hyprland = inputs.hyprland.packages.${prev.system}.hyprland;
          xdg-desktop-portal-hyprland = inputs.hyprland.packages.${prev.system}.xdg-desktop-portal-hyprland;
        })
      ] ++ [
        (final: prev: { quickshell = pkgs-unstable.quickshell; })
        (final: prev: { yabai = pkgs-unstable.yabai; })
        (final: prev: { jankyborders = pkgs-unstable.jankyborders; })
        (final: prev: { neovim = pkgs-unstable.neovim; })
      ] ++ lib.optionals hasWezterm [
        (final: prev: { wezterm = inputs.wezterm.packages.${prev.system}.default; })
      ] ++ lib.optionals hasPkgsOtbr [
        (final: prev: { openthread-border-router = pkgs-otbr.openthread-border-router; })
      ];
  };
  networking.hostName = hostName;
  time.timeZone = "Asia/Shanghai";
  users.users.${userName} = {
    name = userName;
    shell = pkgs.zsh;
  };
  programs.zsh.enable = true;
}
