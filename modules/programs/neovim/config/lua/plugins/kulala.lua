...
return {
  ...
  ft = "http, rest",
  opts = {
    display_mode = "split", -- FIXME: "split"模式会和nvchad有兼容问题
  },
  config = function()
    vim.api.nvim_create_autocmd("FileType", {
      ...
      callback = function()
        ...
        vim.api.nvim_buf_set_keymap( 0, "n", "<leader>i", "<cmd>lua require('kulala').inspect()<cr>", { noremap = true, silent = true, desc = "Inspect the current request" })
        vim.api.nvim_buf_set_keymap( 0, "n", "<leader>t", "<cmd>lua require('kulala').toggle_view()<cr>", { noremap = true, silent = true, desc = "Toggle between body and headers" })
      end,
    })
  end,
}
